...
subroutine mlrmodel(fvec,n,d,m,c,cgrad)
    ...
    do i1=1,m-1
        do j1 =1,n
            ...
            cgrad(i1 +(m-1)*n) = cgrad(i1 +(m-1)*n)+SUM(a(i1,:)-1.0d0)
            else 
            cgrad(i1 +(m-1)*n) = cgrad(i1 +(m-1)*n)+SUM(a(lr_y(j1),:))
            end if
        end do
    end do
    print * ,"here"
    do i1 = 1,m-1
        do j1=1,n
            j2 = (j1-1)*(m-1)+1
            if (lr_y(j1).EQ.i1) then
            cgrad(j2:j2+m-2) = cgrad(j2:j2+m-2) - SUM(lr_x(j1,:)*(1.0d0-a(i1,:))) + 2.0d0*lr_lambda*w(:,j1)
            else
            cgrad(j2:j2+m-2) = cgrad(j2:j2+m-2) + SUM(lr_x(j1,:)*a(lr_y(j1),:)) + 2.0d0*lr_lambda*w(:,j1)
            end if
        end do
    end do
